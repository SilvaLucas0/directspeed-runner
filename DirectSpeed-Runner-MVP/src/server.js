...
const PORT=Number(process.env.PORT||8787), APP_ID=process.env.BASE44_APP_ID, SECRET=process.env.JWT_SECRET, BASE44_SERVER_URL=process.env.BASE44_SERVER_URL||'https://direct-speed-cloud.base44.app';
...
app.post('/api/auth/exchange',async(req,res)=>{try{const baseToken=(req.headers.authorization||'').replace(/^Bearer\s+/i,'');if(!baseToken)return res.status(401).json({error:'Token Base44 ausente.'});console.log('auth/exchange tentativa: appId='+APP_ID+' tokenLen='+baseToken.length+' tokenStart='+baseToken.slice(0,12));try{const claims=jwt.decode(baseToken)||{};console.log('auth/exchange claims:',JSON.stringify({app_id:claims.app_id,appId:claims.appId,aud:claims.aud,iss:claims.iss,env:claims.env,data_env:claims.data_env,scope:claims.scope,typ:claims.typ,keys:Object.keys(claims)}));}catch(decodeErr){console.log('auth/exchange: falha ao decodificar token',decodeErr.message);}try{const candidates=[['entities/User/me','/apps/'+APP_ID+'/entities/User/me'],['auth/me','/apps/'+APP_ID+'/auth/me'],['users/me','/apps/'+APP_ID+'/users/me']];for(const [label,path] of candidates){try{const probeUrl=BASE44_SERVER_URL+'/api'+path;const resp=await fetch(probeUrl,{headers:{Authorization:'Bearer '+baseToken,'X-App-Id':APP_ID}});const text=await resp.text();console.log('auth/exchange probe ['+label+']',resp.status,text.slice(0,200));}catch(probeErr){console.log('auth/exchange probe ['+label+'] falhou:',probeErr.message);}}}catch(outerProbeErr){console.log('auth/exchange: erro nas sondas',outerProbeErr.message);}const b=createClient({appId:APP_ID,token:baseToken,serverUrl:BASE44_SERVER_URL});const user=await b.auth.me();ensureUser(user);res.json({token:sign(user),user:{id:user.id,email:user.email}});}catch(e){console.error('auth/exchange falhou:',e.status||e.response?.status||'',e.message,JSON.stringify(e.response?.data||e.data||''));res.status(401).json({error:'Não foi possível validar sua conta DirectSpeed.',detail:e.message});}});
